...
public class Record {
    ...
    public Record(ArrayList<Constants.DataTypes> cols, ArrayList<Object> vals, int rowID){
        this.columns = cols;
        this.values = vals;
        this.rowID = rowID;
        ...
        for(int i = 0; i < columns.size(); i++){
           ...
           if(val == null){
                col = Constants.DataTypes.NULL;
           }else{
                //Calculate the size of the value
                Constants.DataTypes datatype = col;
                int size;
                switch(datatype){
                    case TINYINT:
                        size = 1;
                ...
                if (size != -1) {
                    ...
                    recordLength += ((String) val).length();
                }  

           }

           //Set header information based on column type
           if(col != Constants.DataTypes.TEXT){
                pageHeader[i + 1] = (byte) col.ordinal();
           }else{
                pageHeader[i + 1] = (byte) (col.ordinal() + ((String) val).length());
           }
        }
    }
